...
def flt_to_custom_flt(num, EXP_LEN, MNT_LEN, denorm, verb=0): # returns an integer of the form- <'EXP in binary' , 'MNT in binary'>
  ...
  if (verb==1):
    print(exp_fin, mnt, str(format(exp_fin, '0'+str(EXP_LEN)+'b')) + str(format(mnt, '0'+str(MNT_LEN)+'b')))

  return int(str(format(exp_fin, '0'+str(EXP_LEN)+'b')) + str(format(mnt, '0'+str(MNT_LEN)+'b')), 2)

def custom_flt_to_flt(num, EXP_LEN, MNT_LEN, denorm, verb=0): # returns a floating point number from the <'EXP in binary' , 'MNT in binary'> representation
  # Special case of zero
  if(num == 0): ## If all bits of mant and exp are 0, treat it as zero instead of 1.0 * 2^(-EXP_MIN)
    return 0

  bin_num= str(format(num,'0' + str(EXP_LEN+MNT_LEN) +'b'))
  exp_str= bin_num[:EXP_LEN]
  mnt_str= bin_num[EXP_LEN:]
  if verb:
  ...
  final_num= float(mnt * (2**(-MNT_LEN + exp)))
...
#######################
def flt_add (num_1, num_2, EXP_L, MNT_L, denorm, verb=0): # Model for floating-point addition working on custom_float numbers of form <'EXP in binary' , 'MNT in binary'>
